...
public class DataTranslator {
    ...
    public void convertFrom(Player player, Software software) {
        ...
        if (player != null) {
            player.sendMessage(Translate.color("&aConverted from " + software.name() + " took " + timeElapsed + "Ms"));
        } else {
    ...
    public void convertTo(Player player, Software software) {
        ...
        if (player != null) {
            player.sendMessage(Translate.color("&aConverted to " + software.name() + " took " + timeElapsed + "Ms"));
        } else {
    ...
    private IDataTranslator getDataTranslator(CommandSender sender, Software software) {
        if (software == Software.CMI) {
            CMI cmi = this.plugin.getOtherPlugins().getCmi();

            if (cmi == null) {
                if (sender != null) {
                    sender.sendMessage(Translate.color("&cThe plugin CMI must be loaded to convert to/from CMI"));
                    return null;
                } else {
